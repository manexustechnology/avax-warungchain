...
const SellerNavigation: React.FC<SellerNavigationProps> = ({ user, onLogout, isCollapsed, onNavItemClick }) => {
  ...
  // Sellers have full admin privileges - treat them as admins
  // This ensures all menus are shown regardless of database connection
  const userRole = user.role || 'seller'; // Default to seller if role is undefined
  const hasAdminAccess = userRole === 'seller' || userRole === 'admin';
  const hasInventoryAccess = userRole === 'seller' || userRole === 'admin' || userRole === 'inventory';
  const hasCashierAccess = userRole === 'seller' || userRole === 'admin' || userRole === 'cashier';
